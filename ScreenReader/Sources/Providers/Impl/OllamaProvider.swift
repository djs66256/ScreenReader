...
class OllamaProvider: LLMProvider {
    ...
    func send(messages: [Message]) async throws -> AsyncThrowingStream<Message, Error> {
        guard let modelName = modelName else {
            throw NSError(domain: "OllamaProvider", code: 400, userInfo: [NSLocalizedDescriptionKey: "Model name is not configured"])
        }
        guard let baseURL = URL(string: baseURLString ?? "") else {
            throw NSError(domain: "OllamaProvider", code: 400, userInfo: [NSLocalizedDescriptionKey: "API URL is not configured"])
        }
        ...
        struct OllamaURLRequest: URLRequestConvertible {
            let url: URL
            ...
            func asURLRequest() throws -> URLRequest {
                ...
                let parameters: [String: Any] = [
                    "model": modelName,
                    "messages": messages.map { $0.toOllamaFormat() },
                    "stream": true
                ]
...
*/
extension Message {
    func toOllamaFormat() -> [String: Any] {
            ...
            for part in parts {
                ...
                case .imageData:
                    ...
                    if components.count == 2 {
                        images.append(components[1])
                    } else {
                        images.append(part.value)
                    }
                case .imageURL:
                    // Ollama目前不支持直接使用URL，需要先下载并转换为base64
                    break
                }
            }
